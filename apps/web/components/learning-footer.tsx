  ...
  return (
    <footer className="learning-footer mt-16 overflow-hidden border-t border-slate-200/80">
      <div className="container max-w-6xl py-12">
        <section className="glass-panel mb-10 overflow-hidden border border-slate-200/80 bg-white/90 p-8 md:p-10">
          <div className="grid gap-8 lg:grid-cols-[minmax(0,1.1fr),minmax(0,0.9fr)] lg:items-center">
            <div className="min-w-0">
              <div className="mb-3 text-xs font-semibold uppercase tracking-[0.2em] text-cyan-600">The Learning Journal</div>
              <h2 className="max-w-3xl text-4xl font-[Outfit] text-slate-900">
        ...
        <div className="grid gap-8 lg:grid-cols-[minmax(0,1.1fr),minmax(0,0.85fr),minmax(0,1fr),minmax(0,1fr)]">
          ...
          <section className="min-w-0">
            <div className="mb-4 text-xs font-semibold uppercase tracking-[0.18em] text-slate-500">Navigate</div>
            <div className="space-y-3 text-sm text-slate-600">
              ...
              <a href="/learn#collections" className="block hover:text-slate-900 transition">Collections</a>
          ...
          <section className="min-w-0">
            <div className="mb-4 text-xs font-semibold uppercase tracking-[0.18em] text-slate-500">Collections</div>
            <div className="space-y-4">
              {collections.map((collection) => (
                <a
                  key={collection.id}
                  href={collection.href}
                  className="block min-w-0 rounded-2xl border border-slate-200 bg-white/80 p-4 transition hover:border-slate-300 hover:bg-white"
                >
                  <div className={`mb-2 text-[11px] font-semibold uppercase tracking-[0.18em] ${themeClass(collection.theme).split(" ")[0]}`}>
                    {collection.label}
                  </div>
                  <div className="mb-2 text-base font-medium text-slate-900">{collection.articleCount} live guides</div>
          ...
          <section className="min-w-0">
            <div className="mb-4 text-xs font-semibold uppercase tracking-[0.18em] text-slate-500">Latest Reads</div>
            <div className="space-y-4">
              {latestArticles.map((article) => (
                ...
                >
                  ...
                  <div className="mb-2 text-base font-medium text-slate-900">{article.title}</div>
